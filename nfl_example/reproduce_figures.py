"""Reproduce manuscript figures from verified derived outputs.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Check inputs for figure reproduction.")
...
def main() -> None:
    ...
    if args.dry_run:
        print("reproduce_figures interface check passed; no figures were regenerated.")
        return
    if not args.derived_output_root.exists():
        raise SystemExit(f"Derived output root does not exist: {args.derived_output_root}")
    raise SystemExit("Full NFL figure reproduction is not verified in this public scaffolding package.")
